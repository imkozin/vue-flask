<template>
  ...
  <b-form @submit.prevent="submitLogin" class="custom-border p-5">
    ...
    <b-form-group id="input-group-1" label="Login (Email):" label-for="input-1">
      <b-form-input
        id="input-1"
        v-model="form.login"
        type="text"
...
export default {
  ...
  methods: {
    async submitLogin() {
        try {
            const response = await axios.post('https://flask-server-6kzx.onrender.com/api/login', this.form);
